...
{
    public class DadEstado : DadPadrao
    {
        private ClsEstado oDTO;
        public static readonly string nomeTabela = "Tb_Estado";
        ...
        public bool ValidarRelacionamento(ref ClsBancoDados banco, long Id)
        {
            try
            {
                ...
                sql = $" SELECT Id FROM Tb_Cidade WHERE IdEstado = '{Id}'";
                sql += $" AND Id <> {Id}";
        ...
        public void Filtra(ref ClsBancoDados banco, ref ClsEstado obj)
        {
            try
            {
        ...
        private void FiltraRelacionamento(ref ClsBancoDados banco, ref ClsEstado obj)
        {
            try
            {
                if (obj.Pais.Id > 0)
                {
                    var oPais = obj.Pais;
                    new DadPais().Filtra(ref banco, ref oPais);
                }
            }
            catch (Exception ex)
            {
        ...
        public ClsEstado MontaObjeto(DataRow dr)
        {
            try
            {

                var obj = new ClsEstado();
                ...
                obj.Pais.Id = dr.GetValue("IdPais").ToInteger();
        ...
        public DataTable TodosPorPais(ref ClsBancoDados banco, bool Ativos, int IdPais)
        {
            DataTable dt = new DataTable();
            ...
            sql += " WHERE 1 = 1";
            if (Ativos) sql += $" AND Status = 1";
            if (IdPais > 0) sql += $" AND IdPais = {IdPais}";
        ...
        public List<ClsEstado> ListaTodos(ref ClsBancoDados banco, bool Ativos)
        {
            var lista = new List<ClsEstado>();
